...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Mirror the PAT save path: inject into process.env so resolveGitHubToken()
    // returns the new token immediately, even if a stale GITHUB_TOKEN is already
    // set in the environment (e.g. from .env.local).
