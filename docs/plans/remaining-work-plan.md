...
- Phase 0: Strategy doc + full canonical schema ([dual-llm-integration-strategy.md](../strategy/dual-llm-integration-strategy.md))
...
agentic-flow runs **in-process** on the developer's machine. No remote host, no MCP-over-HTTP, no webhooks, no status polling. Dossier uses agentic-flow via subprocess adapter (CLI: `npx agentic-flow --agent coder --task "<task>"`).

The `AgenticFlowClient` interface (`dispatch`, `status`, `cancel`) is preserved — the implementation uses subprocess spawn. The mock client works for development.
...
- **RuVector** — embedded in agentic-flow or imported directly. Local vector storage.
...
2. **Populate:** Creates activities, steps, and cards for each workflow; prompt instructs the LLM to ask the user to define actions (View Details & Edit, Monitor, Reply, Test, Build, or custom).
