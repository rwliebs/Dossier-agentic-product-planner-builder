...
-- Idempotent: safe to run after 20240213000001 or 20250212000000 (see supabase/MIGRATION_OVERLAP.md)

-- Enums (idempotent so this can run after slice_a_enums_and_foundation or slice_a_b_base)
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_workflow_activities_position ON workflow_activities(workflow_id, position);
...
CREATE INDEX IF NOT EXISTS idx_steps_workflow_activity_id ON steps(workflow_activity_id);
CREATE INDEX IF NOT EXISTS idx_steps_position ON steps(workflow_activity_id, position);
...
CREATE INDEX IF NOT EXISTS idx_cards_step_priority ON cards(step_id, priority);
...
CREATE INDEX IF NOT EXISTS idx_planning_actions_project_id ON planning_actions(project_id);
CREATE INDEX IF NOT EXISTS idx_planning_actions_created_at ON planning_actions(project_id, created_at DESC);
